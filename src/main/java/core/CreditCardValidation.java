...
public class CreditCardValidation {
	...
	public static boolean type(String cc_type, String cc_number) {
		...
		if (number.startsWith("4") && (number.length() == 13 || 
number.length() == 16) && cc_type.equals("VISA")) 
		{return true;} 

		else if ((number.startsWith("51") || 
number.startsWith("52") || number.startsWith("53") || number.startsWith("54") ||
...
number.length() == 15 && cc_type.equals("American Express")) {return true;}
	...
	public static boolean exp(String cc_exp) {             // 2019-07-07 21:33:11.045
		...
		int today = Integer.parseInt((String.valueOf(ts.toString().split("-")[0].
substring(2, 4)) + ts.toString().split("-")[1])); return today <= exp;
}
